({
     constructValidationMap : function(component){
        var valMap={};
        valMap["contact.OtherCity"]="Required,StrictlyText";
        valMap["contact.OtherState"]="Required,StrictlyText";
        valMap["contact.OtherStreet"]="StrictlyText";
        valMap["contact.OtherPostalCode"]="Email";     
         ...
         for(var i in valList){
             var tmpVal = value ;
                if(valList[i]=='Required'){
        			if(fieldAuraId === 'contact.OtherCity' || fieldAuraId === 'contact.OtherState'){
             ...
             if(valList[i]=='StrictlyText'){
        			if(fieldAuraId === 'contact.OtherCity' || fieldAuraId === 'contact.OtherCity'){
                    	tmpVal = (!$A.util.isUndefinedOrNull(tmpVal) && !$A.util.isEmpty(tmpVal)) ? tmpVal.trim() : tmpVal;
    ...
    checkValidity : function(component, isDraft, valMap){
     ...
     saveData : function(component, sObjList, saveOn){ 
        ...
        var contact = component.get("v.contact");
         ...
         alert('Ankush-->'+contactID);
          component.set("v.contact.Id",contactID);
         console.log(contact);
    ...
    saveToServer : function (component, helper, sObjList, saveOn, hCallBack){
     var feature = 'customerdemographicdetails';
        if(!$A.util.isUndefinedOrNull(sObjList)) {
            ...
            util.getDataFromServer(component, null, {"sObjList":sObjList,"feature":feature}, "c.saveRecordsAddress", helper, function(result, helper) {
